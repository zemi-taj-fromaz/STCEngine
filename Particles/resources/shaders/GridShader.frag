...
vec4 grid(vec3 fragPos3D, float scale, bool drawAxis) {
    ...
    vec4 color = vec4(1.0) - vec4(0.2, 0.2, 0.2, 1 - min(line, 1.0));
    // z axis
...
// Discard fragments that are close to white
if (all(lessThanEqual(abs(outColor.rgb - vec3(1.0)), vec3(0.01))) || all(lessThanEqual(abs(outColor.rgb - vec3(0.8)), vec3(0.01)))) {
    discard;
}
}
