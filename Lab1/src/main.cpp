...
public:
	ExampleLayer() : Layer("Example") 
	{
		...
		//	this->m_ComputePipeline = std::make_shared<Pipeline>(pipelineLayoutCompute, computeShaderName);
			...
			create_pipelines({ plainPipeline, skyboxPipeline
		...
		//------------------------- TEXTURES ---------------------------------------------------------------
			...
			create_textures({ skyboxTex // , rustAlbedoTex, rustNormalsTex, rustMetallicTex, rustRoughnessTex
	});
		...
		auto cat = std::make_shared<MeshWrapper>(plainPipeline, catMesh);
		cat->animated = "spiral.txt";
		cat->scale = glm::scale(glm::mat4(1.0f), glm::vec3(0.1f, 0.1f, 0.1f));
